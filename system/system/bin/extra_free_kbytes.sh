...
then
    ...
    if [ $RamSizeGB -gt 6 ]; then
        max_watermark_scale=$((vendor_watermark_scale_factor + 10))
    fi
...
# MIUI ADD: Performance_MemoryEnhance
function configure_ufs_swap_memcg()
{
        memTotal=($(grep MemTotal /proc/meminfo | awk '{print $2}'))

        if [ $memTotal -gt 15000000 ] && [ $memTotal -lt 17000000 ]; then
                if [ -e /dev/memcg/memory.xswapd.watermark ]; then
                        echo "3500 3500 3000 75" > /dev/memcg/memory.xswapd.watermark
                fi
        fi
}
configure_ufs_swap_memcg
# END Performance_MemoryEnhance
